...
use Monitoring\Exception\QueryServerException;
...
class SampQuery
{
    ...
    function Connect( $szServerIP, $iPort )
    {
            ...
            $this->rSocketID = @fsockopen( 'udp://' . $this->szServerIP, $iPort, $iErrorNo, $szErrorStr, 10 );
            if (!$this->rSocketID) {
                    throw new QueryServerException( 'Cannot connect to the server: ' . $szErrorStr );
            }

            stream_set_timeout( $this->rSocketID, 10 );
            $this->bStatus = true;
    }
    ...
    function GetPlayers( )
    {
            ...
            {
                    $aReturnArray[ ] = array (
                            'Name' => $this->GetPacket( 1 ),
                            'Frags'    => $this->toInteger( fread( $this->rSocketID, 4 ) )
                    );
            }
